...
void floyd(Graph m, Graph nearest_path)
{
    foreach (k, m.rows)
    {
        foreach (i, m.rows)
        {
            foreach (j, m.rows)
            {
                if (m.p[i][k] + m.p[k][j] < m.p[i][j])
                {
                    m.p[i][j] = m.p[i][k] + m.p[k][j];
                    nearest_path.p[i][j] = k;
                }
            }
        }
    }
...
void print_path_floyd(Graph next, int v0, int v1)
{
    int k = next.p[v0][v1];
    if (k == -1)
    {
    ...
    print_path_floyd(next, v0, k);
    printf("%d\n", k);
    print_path_floyd(next, k, v1);
}
...
void main()
{
    ...
    Graph nearest_path = createIdxGraph(N, N);
    scanfGraph(m);
    floyd(m, nearest_path);
    int M;
    scanf("%d", &M);
    foreach (i, M)
    {
        ...
        scanf("%d %d", &v0, &v1);
        printf("%d\n", v0);
        print_path_floyd(nearest_path, v0, v1);
        printf("%d\n", v1);
    }
    freeGraph(nearest_path);
    freeGraph(m);
}